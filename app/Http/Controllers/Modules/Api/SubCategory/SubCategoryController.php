	    ...
	    */
	    public function subCategoryListMy(Request $request)
	    {
	        ...
	        if(count($chkproduct)>0)
	        {
		        ...
		        {
		        	$getsubcategory = ProductSubCategory::orderBy('id','DESC')->with('getCategoryDetails')->offset($offsatval)
		                            ->limit($limit)->get();
		        }
		        else
		        {
		        	$getsubcategory = ProductSubCategory::orderBy('id','DESC')->with('getCategoryDetails')->get();
		        }
		    			...
		    			if (isset($request->search_keyword)) 
				            {
				            	...
				            	$getsubcategory = ProductSubCategory::where(function($q) use($request){
					                   $q->where('sub_cat_name',$request->search_keyword);
					                });
				            		 
				            		if(!empty($offsatval || $limit) ){
				            			$getsubcategory->offset($offsatval)
				            						->limit($limit);
				            		}
				            		
				            		$getsubcategory = $getsubcategory->with('getCategoryDetails')->get();
